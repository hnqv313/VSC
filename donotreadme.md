...
- **Cấu hình siêu linh hoạt (`config.py`):** Dễ dàng điều chỉnh độ khắt khe của bộ lọc (`cutoff`), số lượng ứng viên (`top_n`), trọng số n-gram, hoặc bật/tắt phần thưởng chính xác (`exact_match_bonus`).
